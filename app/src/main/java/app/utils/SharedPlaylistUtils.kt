...
object SharedPlaylistUtils {
    ...
    private fun WatchActivity.saveFolderPathAsMediaDirectory(uri: Uri) {
     ...
     * is heavy.
     */
    private fun WatchActivity.retrieveFile(fileName: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            if (fileName.contains("http://", true) ||
                ...
                player?.injectVideo(this@retrieveFile, fileName.toUri(), isUrl = true)
            } else {
