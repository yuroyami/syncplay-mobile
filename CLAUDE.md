...
`HomeScreen.kt` - one adaptive form of four blocks (identity, server, engine, join) placed by the window: one spread column when narrow, a centred two-column block when wide and tall, two dense pinned columns on a phone on its side. Fields are the drawn `Field` control with inline help and errors. The server block is a segmented choice: Official (`syncplay.pl`, port row 8995-8999), Custom, or Host mine (hosting panel inline, joins `127.0.0.1`). The engine block is `HomeEnginePicker` (a segmented frame showing every engine with a badge; the "?" under the selection unfolds a per-engine story card - the old wheel is gone). Join is the one gradient key with the save-shortcut glyph inside it; username/room are stripped of backslashes, trimmed and capped (149/34) in the shared join builder. `HomeViewmodel.kt` - `joinRoom(config?)`; null = solo mode. `JoinConfig.kt` - `@Serializable user/room/ip/port/pw`; defaults ip `syncplay.pl`, port 8997; `save()` only when `REMEMBER_INFO`. Components: `HomeTopBar` (wordmark + logo→About, theme picker, settings glyph → `Screen.Settings`), `PopupAPropos` (tagline, facts, Watch-alone row via `joinRoom(null)`, source/report links), `PopupDidYaKnow` (first-launch tips with a working Next; `NEVER_SHOW_TIPS`).
