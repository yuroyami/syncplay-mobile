...
| Shared-state SSOT | KiteSSOT plugin `io.github.yuroyami.kitessot` 4.2.0 (appName/version/bundleId/SDKs/logo; values fed from buildSrc `AppConfig`) |
...
**`exoOnly` flag** is the single source of truth resolved by `AppConfig.resolveExoOnly(providers)` (overridable with `-PexoOnly=true`), used by root KiteSSOT to emit the commonMain `EXOPLAYER_ONLY` BuildConfig field and by `androidApp` for flavor selection. `exoOnly` switches the applicationId to `com.reddnek.syncplay`, skips the mpv native build scripts, and ships no native player libs.
...
**KiteSSOT migration (DONE, 2026-07-24; moved to the 3.0 DSL on 2026-08-19 and the 4.x DSL on 2026-08-29).** The SSOT plugin is `io.github.yuroyami.kitessot` 4.2.0 (Gradle Plugin Portal; successor of kmp-ssot). Root `build.gradle.kts` holds the whole `kiteSsot { }` config, fed from `AppConfig`. Key semantics vs the old kmp-ssot:
- Identity applies in AGP `finalizeDsl` (AFTER module DSL), so the exoOnly applicationId swap lives in the root block (`id(if (exoOnly) …) { }`), NOT in androidApp's `defaultConfig`. A module-level override cannot win anymore.
...
- versionCode scheme changed in KiteSSOT 3.0 to `1 | major(3) | minor(3) | patch(2) | rebuild(1)`, so 0.23.2 moved from `1000023002` to `1000023020`. Every new code is LARGER than the 2.x code for the same version, so Play monotonicity holds. Ten codes now exist per version: bump `android { rebuild }` / `ios { rebuild }` for a re-upload instead of faking a patch release.
- Branding is on-demand: launcher assets regenerate ONLY via `kiteRewriteLogo` (no more per-build icon stomping / launcher-PNG git noise). The task transactionally installs both Android and iOS assets and, because `logo { rewrite { replaceOld = true } }` is configured, takes over known legacy Android collisions after backing them up. `compileSdk >= 33` makes it emit `mipmap-anydpi-v33` wrappers with the themed-icon `<monochrome>` layer (issue #143); the plugin owns all launcher outputs and their ownership manifests.
- `kiteDoctor` / `kiteCheck` / `kitePlan` are the diagnostics; `kiteVerify` reports the resolved model. `kiteCheck` is the release-grade ownership/configuration gate.
- iOS source mutation is NOT automatic. In 4.x, `ios { rewrite { } }` arms `kiteRewriteXcode`, while `logo { rewrite { } }` arms `kiteRewriteLogo`; neither runs during a normal build. On a version bump run `kiteRewriteXcode` (pbxproj identity/version/knownRegions; leaves a gitignored `*.kitessot.bak`), and after logo changes run `kiteRewriteLogo` (regenerates Android launcher resources plus the opaque 1024 universal `AppIcon.appiconset`). The catalog is `AppIcon`; `logo.appiconset` is an unreferenced leftover. Plugin state (`.kitessot/` dirs, `*.kitessot.bak`) is gitignored: regenerable local state, never assets.
- kitessot 2.0.1 shipped the AGP 9.2.x compat widening, and 2.0.2 fixed a real-world config-time bug this migration uncovered (resilient-invocation detection realized requested tasks from `plugins.withId`, observing `:shared`'s compile classpaths before its script body ran, surfacing as "configuration was observed" / "property 'languageVersion' is final" when requesting `:shared:compileAndroidMain` directly). 2.0.3 widened AGP support to 9.3.x. Do not downgrade below 4.2.0 without also reverting the 4.x DSL.
- The old KMPS021 pbxproj drift was cleared with `kiteRewriteXcode` on 2026-08-29: `CURRENT_PROJECT_VERSION` is now `1000023020`, the selected `iosApp` target is aligned, and `kiteCheck` passes with zero errors and zero warnings.

**Launcher icon ownership (themed icon, issue #143).** KiteSSOT owns every launcher asset: the per-density PNGs, the `mipmap-anydpi-v26` wrappers, and the `mipmap-anydpi-v33` wrappers whose `<monochrome>` layer (referencing `@mipmap/ic_launcher_foreground` — the launcher only uses its alpha) provides the Android 13+ themed icon. The canonical design artwork is the pure-vector `art/synkplay_logo.svg`, with the approved colors and gradient geometry recorded in `art/synkplay_logo_palette.md`; Compose uses its equivalent Android VectorDrawable `synkplay_logo.xml` because Android's Compose resource decoder rejects SVG, while `synkplay_fg.png` is the transparent 1024×1024 KiteSSOT raster used with the opaque `synkplay_bg.png` platform plate. Regeneration is ON-DEMAND via `kiteRewriteLogo` — normal builds never touch source icons. Do NOT hand-edit generated launcher files; edit the canonical SVG, regenerate the VectorDrawable and `synkplay_fg.png`, then rerun the task.
