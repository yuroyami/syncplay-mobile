...
import app.SyncplayApp.Companion.booleanPrefs
import app.SyncplayApp.Companion.intPrefs
import app.SyncplayApp.Companion.readBoolean
import app.SyncplayApp.Companion.readInt
import app.SyncplayApp.Companion.readString
import app.SyncplayApp.Companion.stringPrefs
import app.home.ComposeUtils.FancyIcon
import app.home.ComposeUtils.MultiChoiceDialog
...
class Setting(
    ...
    /** Coupling this setting with the datastore global instance */
    init {
        when (type) {
            SettingType.OneClickSetting -> {}
            SettingType.MultiChoicePopupSetting -> stringPrefs[key] = readString(key, defaultValue as String)!!
            SettingType.CheckboxSetting -> booleanPrefs[key] = readBoolean(key, defaultValue as Boolean)!!
            SettingType.ToggleSetting -> booleanPrefs[key] = readBoolean(key, defaultValue as Boolean)!!
            SettingType.SliderSetting -> intPrefs[key] = readInt(key, defaultValue as Int)!!
        }
    }
    ...
    private fun BooleanSettingUI(modifier: Modifier = Modifier, type: Boolean) {
        val boolean = booleanPrefs[key]?.collectAsState(initial = defaultValue as Boolean)
        ...
        ConstraintLayout(
            modifier = modifier
                ...
                .clickable(
                    ...
                    scope.launch {
                        SyncplayApp.writeBoolean(key, !boolean?.value!!)
                    }
                }
        ) {
            ...
            if (type) {
                Checkbox(
                    ...
                    onCheckedChange = { b ->
                        scope.launch {
                            SyncplayApp.writeBoolean(key, b)
                        }
                    },
                    modifier = m
                )
     ...
     * @exception SettingCreationException When [entryPairs] is not passed/specified. */
    @Composable
    private fun ListSettingUI(modifier: Modifier = Modifier) {
        val dialogOpen = remember { mutableStateOf(false) }
        val selectedItem = stringPrefs[key]?.collectAsState(initial = defaultValue as String)
        val scope = rememberCoroutineScope()
        ...
        if (dialogOpen.value) {
            MultiChoiceDialog(
                ...
                onItemClick = { i ->
                    ...
                    scope.launch {
                        SyncplayApp.writeString(key, entryValues[i])
                    }
                })
        }
    ...
    private fun SliderSettingUI(modifier: Modifier = Modifier) {
        val value = intPrefs[key]?.collectAsState(initial = defaultValue as Int)
        val scope = rememberCoroutineScope()
        ...
        ConstraintLayout(
            ...
            Slider(value = value?.value?.toFloat() ?: 0f,
                valueRange = (minValue.toFloat())..(maxValue.toFloat()),
                onValueChange = { f ->
                    scope.launch {
                        SyncplayApp.writeInt(key, f.toInt())
                    }
                }, modifier = Modifier
                    .fillMaxWidth()
