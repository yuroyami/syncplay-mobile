...
val EnStrings = object : Strings {
    ...
    override val uiSettingMpvInterpolationSummary = "Reduce jittering by enabling framerate interpolation. Note that this will only be applied if the video sync option is set to one of the 'display' mode, otherwise it will be silently ignored. This may not work well in some cases."
    override val uiSettingMpvVidsyncTitle = "Video sync mode"
    override val uiSettingMpvVidsyncSummary = "Controls how mpv syncs the video, either to audio (default, recommended) or the display itself. Setting this to anything but 'audio' may cause issues on displays with high frame refresh rates (>60 FPS). Consider lowering your device's framerate accordingly."
    override val uiSettingMpvProfileTitle = "Rendering preset (profile)"
    override val uiSettingMpvProfileSummary = "Configures mpv capabilities according to one of the built-in profile presets. Default is 'fast' as it is phone-friendly. Using 'low-latency' can be good when streaming from live sources."
