...
object PlaylistUtils {
    ...
    fun addURLs(string: List<String>) {
        ...
        viewmodel!!.p.sendPacketAsync(JsonSender.sendPlaylistChange(l))
    }
    ...
    fun addFiles(uris: List<String>) {
        for (uri in uris) {
            ...
            if (viewmodel!!.p.session.sharedPlaylist.isEmpty() && viewmodel!!.p.session.spIndex.intValue == -1) {
                ...
                viewmodel?.p?.sendPacketAsync(JsonSender.sendPlaylistIndex(0))
            }
            viewmodel!!.p.session.sharedPlaylist.add(filename)
        }
        viewmodel!!.p.sendPacketAsync(JsonSender.sendPlaylistChange(viewmodel?.p?.session!!.sharedPlaylist))
    }
    ...
    fun clearPlaylist() {
        ...
        viewmodel!!.p.sendPacketAsync(JsonSender.sendPlaylistChange(emptyList()))
    }
    ...
    fun sendPlaylistSelection(index: Int) {
        viewmodel?.p?.sendPacketAsync(JsonSender.sendPlaylistIndex(index))
    }
